...
def is_domain_valid(dial_url):
    ...
    try:
        ...
        resp = requests.get(dial_url, verify=False)
...
def auto_login_paas(paas_domain, username, password):
    ...
    # Django's CSRF protection uses referer header checking in addition to checking a hidden form field against a cookie
...
def start_deploy_app_task(paas_domain, appid, env, is_use_celery=True, is_use_celery_beat=True):
    ...
    resp = session.post(
        deploy_app_url, data=deploy_param, verify=False, headers={"X-CSRFToken": csrftoken, "Referer": app_info_url}
    )
    ...
    try:
        ...
        print((resp.text))
        raise DeployException("start deploy {} to {} exception:{}".format(appid, env, e.message,))
...
def poll_deploy_task(paas_domain, appid, event_id):
    ...
    retry = 32
    while retry > 0:
        resp = session.get(task_result_url, verify=False)
        resp_data = resp.json()
        # 部署成功
...
if __name__ == "__main__":
    ...
    parser.add_argument("-d", "--domain", help="bk paas domain", required=True)
...
===========================
""".format(
                APP_ID, ENV, USERNAME, PASSWORD, PAAS_DOMAIN
            )
        )
    )
