...
class BkJwtBackend(ModelBackend):
    ...
    def authenticate(self, request=None):
        ...
        try:
            ...
            logger.exception("自动创建 & 更新 User Model 失败: %s" % e)
            return None
    ...
    def verify_bk_jwt_request(request):
        ...
        if not bkoauth_jwt_client_exists:
            ret['message'] = 'bkoauth暂不支持JWT协议'
            return ret

        jwt = JWTClient(request)
        if not jwt.is_valid:
            ret['message'] = "jwt_invalid: %s" % jwt.error_message
            return ret

        # verify: user && app
        app = jwt.get_app_model()
        if not app['verified']:
            ret['message'] = app.get('valid_error_message', 'APP鉴权失败')
            ret['data']['app'] = app
            return ret
        ...
        if not use_esb_white_list and not user['verified']:
            ret['message'] = user.get('valid_error_message', '用户鉴权失败且不支持ESB白名单')
            ret['data']['user'] = user
            return ret
