...
get_config () {
    ...
    local config=(agent.conf btsvr.conf transit.conf)
...
setup_proxy () {
    ...
    for f in {agent,btsvr,transit}.conf; do
        if [[ -f $TMP_DIR/$f ]]; then
...
download_pkg () {
    ...
    cd "$TMP_DIR" && rm -f "$PKG_NAME" {agent,btsvr,transit}.conf."$LAN_ETH_IP"
...
# main program
while getopts I:i:l:s:uc:r:x:p:n:N:g:v:oT:RO:E:A:V:B:S:Z:K: arg; do
    case $arg in
