...
# DEFAULT DEFINITION
NODE_TYPE=agent
PKG_NAME=gse_client-linux-$(uname -p).tgz
...
check_polices_proxy_to_upstream () {
    ...
    for ip in "${UPSTREAM_IP[@]}"; do
        log check_env - "check if it is reachable to port $IO_PORT,$DATA_PORT,$BTSVR_THRIFT_PORT of $ip(${UPSTREAM_TYPE:-GSE_SERVER})"
        is_target_reachable "$ip" "$IO_PORT"
        is_target_reachable "$ip" "$DATA_PORT"
...
setup_crontab () {
    ...
    (
        ...
        echo "*/2 * * * * $AGENT_SETUP_PATH/bin/gsectl watch >/dev/null 2>&1"
    ) >"$tmpcron"
...
# main program
while getopts I:i:l:s:uc:r:x:p:n:N:v:oT:RDO:E:A:V:B:S:Z:K: arg; do
    case $arg in
        I) LAN_ETH_IP=$OPTARG ;;
        i) CLOUD_ID=$OPTARG ;;
