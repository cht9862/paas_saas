...
def arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("-n", "--upstream-ip", type=str, help="comma seperated uptream ip list")
    ...
    for (login_ip, lan_eth_ip, user, port, identity, cloud_id, node_type, _os, tmp_dir,) in hosts:
        construct_cmd = {
            "aix": [
                ...
                "nohup ksh {dest_dir}setup_agent.ksh -T {dest_dir} -i {} -I {} -s {} -c {} -l {} -r {} -p {} -n {} "
                "-x {} -N PROXY {} -O {} -E {} -A {} -V {} -B {} -S {} -Z {} -K {} 2>&1 &".format(
                    cloud_id,
                    lan_eth_ip,
                    args.task_id,
                    args.token,
            ...
            "linux": [
                ...
                "nohup bash {dest_dir}setup_agent.sh -T {dest_dir} -i {} -I {} -s {} -c {} -l {} -r {} -p {} -n {} "
                "-x {} -N PROXY {} -O {} -E {} -A {} -V {} -B {} -S {} -Z {} -K {} 2>&1 &".format(
                    cloud_id,
                    lan_eth_ip,
                    args.task_id,
            ...
            "windows": [
                ...
                '-n "{}" -x {} -N PROXY {} -O {} -E {} -A {} -V {} -B {} -S {} -Z {} -K {}'.format(
                    cloud_id,
                    lan_eth_ip,
                    args.task_id,
                    args.token,
        ...
        if cpu_percent > 90 or memory_percent > 90:
            ...
            Popen(
                f"sleep {sleep_time} && echo '{json.dumps(hosts)}' > {args.json} && " + " ".join(sys.argv), shell=True,
            )
            sys.exit(0)
