...
report_step_status () {
    ...
    tmp_json_body=$(touch /tmp/nm.reqbody."$(date +%Y%m%d_%H%M%S)".`date +%s`.json)
    tmp_json_resp=$(touch /tmp/nm.reqresp."$(date +%Y%m%d_%H%M%S)".`date +%s`.json)
...
is_port_listen () {
    ...
    for i in {0..15}; do
        sleep 1
        for port in "$@"; do
            netstat -aon |grep "${port}" |grep LISTEN 2>/dev/null 2>/dev/null && return 0
        done
    done
...
check_polices_agent_to_upstream () {
    ...
    for ip in "${UPSTREAM_IP[@]}"; do
        log check_env - "check if it is reachable to port $IO_PORT,$DATA_PORT,$FILE_SVR_PORT,$BT_PORT-$TRACKER_PORT of $ip($UPSTREAM_TYPE)"
...
check_policies_proxy_to_upstream () {
    ...
    for ip in "${UPSTREAM_IP[@]}"; do
        log check_env - "check if it is reachable to port $IO_POR,$DATA_PORT,$BTSVR_THRIFT_PORT of $ip(${UPSTREAM_TYPE:-GSE_SERVER})"
        is_target_reachable "$ip" "$IO_PORT"
        is_target_reachable "$ip" "$DATA_PORT"
...
get_config () {
    ...
    for f in "${config[@]}"; do
        http_status=$(http_proxy=$HTTP_PROXY https_proxy=$HTTPS_PROXY curl -X GET '"$CALLBACK_URL"/get_gse_config/?bk_cloud_id="$CLOUD_ID"&filename="$f"&node_type="$NODE_TYPE"&inner_ip="$LAN_ETH_IP"' -o "$TMP_DIR/$f"  --silent -w "%{http_code}") 
        if [[ "$http_status" != "200" ]] && [[ "$http_status" != "000" ]]; then
            fail get_config FAILED "request config $f failed. request info:$CLOUD_ID,$LAN_ETH_IP,$NODE_TYPE,$f. http status:$http_status"
        fi
    done
}
...
setup_agent () {
    ...
    # setup config file
    get_config
    if [[ -f $TMP_DIR/agent.conf ]]; then
        cp -fp "$TMP_DIR/agent.conf" agent/etc/agent.conf
    else
        fail setup_agent FAILED "agent config file lost. please check."
    fi
...
check_deploy_result () {
    ...
    is_port_listen $(seq "$BT_PORT_START" "$BT_PORT_END") || { fail check_deploy_result FAILED "bt port is not listen"; ((ret++)); }
...
# main program
while getopts I:i:l:s:uc:r:x:p:n:N:v:oT:RO:E:A:V:B:S:Z:K: arg; do
    case $arg in
        I) LAN_ETH_IP=$OPTARG ;;
        i) CLOUD_ID=$OPTARG ;;
        ...
        n) UPSTREAM_IP=$(echo "$OPTARG" | awk -F , '{print$1}') ;;
        n) UPSTREAM_IP=
