...
if "%1" EQU "-n" (set UPSTREAM_IP=%~2) && shift && shift && goto CheckOpts
...
:check_polices_pagent_to_upstream
    ...
    call :print INFO check_env - "check if it is reachable to port %IO_PORT%,%DATA_PORT%,%FILE_SVR_PORT%,%BT_PORT%-%TRACKER_PORT% of %ip% GSE PROXY"
    echo=
    for %%p in (%UPSTREAM_IP%) do (
        for %%a in (%IO_PORT%,%DATA_PORT%,%FILE_SVR_PORT%,%BT_PORT%) do (
...
:check_polices_agent_to_upstream
    ...
    call :print INFO check_env - "check if it is reachable to port %IO_PORT%,%DATA_PORT%,%FILE_SVR_PORT%,%BT_PORT%-%TRACKER_PORT% of %UPSTREAM_IP% GSE_SERVER"
    echo=
    call :multi_report_step_status
    for %%p in (%UPSTREAM_IP%) do (
...
:get_config
    ...
    call :multi_report_step_status
    set PARAM="{\"bk_cloud_id\":%CLOUD_ID%, \"filename\":\"agent.conf\", \"node_type\":\"%NODE_TYPE%\", \"inner_ip\":\"%LAN_ETH_IP%\", \"token\":\"%TOKEN%\"}"
    for %%p in (agent.conf) do (
        if "%HTTP_PROXY%" == "" (
            %TMP_DIR%\curl.exe -o %TMP_DIR%\agent.conf --silent -w "%%{http_code}" -X POST %CALLBACK_URL%/get_gse_config/ -d %PARAM%  1>%TMP_DIR%\nm.test.HHHHHHH 2>&1
        ) else (
            %TMP_DIR%\curl.exe -o %TMP_DIR%\agent.conf --silent -w "%%{http_code}" -X POST %CALLBACK_URL%/get_gse_config/ -d %PARAM% -x %HTTP_PROXY% 1>%TMP_DIR%\nm.test.HHHHHHH 2>&1
        )
...
:setup_agent
    ...
    call :get_config
    if exist %TMP_DIR%\agent.conf (
        copy /Y %TMP_DIR%\agent.conf %AGENT_SETUP_PATH%\agent\etc\agent.conf 1>nul 2>&1
    ) else (
        call :print FAIL setup_agent FAILED "agent config file lost. please check."
        call :multi_report_step_status
    )
