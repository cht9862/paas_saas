    ...
    <template>
      ...
      <section class="task-table">
        <bk-table
          ...
          <bk-table-column width="185" :label="$t('执行时间')" prop="startTime"></bk-table-column>
...
export default {
  ...
  computed: {
    ...mapGetters(['selectedBiz', 'fontSize'])
  },
  watch: {
    searchSelectValue: {
      handler() {
  ...
  methods: {
    ...mapActions('task', ['requestHistoryTaskList', 'getFilterCondition']),
    /**
     * 获取搜索条件
     */
    async handleGetConditionData() {
      this.filterData = await this.getFilterCondition()
    },
    /**
     * 获取请求参数
     */
    ...
    async getHistortList() {
      ...
      const res = await this.requestHistoryTaskList(params)
    ...
    async handlePollData() {
      const data = await this.requestHistoryTaskList({
        page: 1,
