    ...
    <section class="log-side-nav" ref="sideContent">
      <div class="log-nav-select">
        <bk-input
          ...
          :placeholder="$t('多个IP以逗号分隔')"
          v-model.trim="search"
          @change="handleSearch">
        </bk-input>
      </div>
      <div class="log-nav-wrapper" ref="logSide" @scroll="handleSideScroll">
        <div class="log-nav-head" :style="ghostStyle" v-if="virtualScroll"></div>
        <div class="log-nav-body" ref="content">
          <ul class="nav-list">
        ...
        <div class="step-wrapper">
          <div class="outer-border-cover">
            <bk-table
              ...
              <bk-table-column :label="$t('步骤')" :resizable="false" min-width="200">
              ...
              <bk-table-column class-name="column-subscript" :width="hasOperaCell ? 100 : 30" prop="opera">
                <template #default="{ row, $index }">
                  <!--
                  <bk-button
                    ...
                    <loading-icon v-if="atomLoading"></loading-icon>
                    <span v-else>{{ $t('单步重试') }}</span>
                  </bk-button>
                  -->
                  <bk-button
                  ...
                  <bk-button ext-cls="step-operation" size="small" v-if="row.isCanSkip">{{ $t('跳过') }}</bk-button>
        ...
        <div :class="['log-detail-wrapper', { 'log-detail-screen': isFullScreen }]">
          <div class="log-detail-content">
            <div class="log-header clearfix">
              ...
              <div>
                <i class="mr20 nodeman-icon nc-export2"
                   v-bk-tooltips="{
                     delay: [300, 0],
                     content: $t('导出日志'),
                     theme: 'log-operate',
...
import { throttle, downloadLog, debounce, isEmpty, takesTimeFormat } from '@/common/util'
...
export default {
  ...
  created() {
    ...
    this.handleSearch = debounce(300, this.filterIpHandle)
  ...
  methods: {
    ...mapActions('task', [
      'requestHistoryTaskDetail',
      'requestHistoryHostLog',
      'requestTaskStop',
      'requestReportLog',
      'requestNodeRetry',
      'requestTaskRetry'
    ...
    async getHostList() {
      ...
      const res = await this.requestHistoryTaskDetail(params)
      if (res) {
        const { list: data, status } = res
        this.jobTypeDisplay = res.jobTypeDisplay || ''
        const list = data.filter(item => item.instanceId && item.status !== 'filtered')
        this.navList.splice(0, 0, ...list)
        this.rawNavList.splice(0, 0, ...list)
        this.hostRuningQueue = /running/ig.test(status) ? [1] : []
        const propHost = list.find((item, index) => {
          if ((this.hostInnerIp && item.innerIp === this.hostInnerIp) || (`${item.instanceId}` === this.instanceId)) {
            this.startIndex = index // 确定当前主机所在下标
            return true
          }
          return false
        })
        if (propHost) {
          ...
          if (this.startIndex) {
            this.$nextTick(() => { // 选中目标移动到可视区域内
              this.$refs.logSide.scrollTop = this.startIndex * this.sideItemHeight
            })
            // 下标为0时的不展示列表的bug
          } else {
            this.endIndex = Math.ceil((this.$refs.logSide.clientHeight) / this.sideItemHeight)
          }
        } else {
    ...
    filterIpHandle() {
      if (this.search) {
        ...
        const navList = this.rawNavList.reduce((arr, item) => {
        ...
        this.navList.splice(0, this.setupNum, ...this.rawNavList)
        this.$nextTick(() => {
          this.$refs.logSide.scrollTop = this.startIndex * this.sideItemHeight
        })
      }
    ...
    async getLogDetail() {
      ...
      const res = await this.requestHistoryHostLog({
        job_id: this.taskId,
        query: {
    ...
    async handleTaskStop(row) {
      ...
      this.runingQueue = []
      const res = await this.requestTaskStop({
        jobId: this.taskId,
    ...
    async handleRetry(row) {
      ...
      const res = await this[requireyKey]({
        jobId: this.taskId,
    ...
    resultHandle(res) {
      ...
      const hasReportRes = res.find(item => this.reportRegExp.test(item.log))
      const needReport = isFailed && !hasReportRes
      this.runingQueue = isSuccess || (isFailed && hasReportRes) ? [] : [-1] // 非执行成功 或 失败且包含上报成功语句 一直轮询
      if (needReport && this.needReport && !this.reportLoading) {
        this.requestReport()
      }
    ...
    handleScroll() {
      ...
      if (!this.isManualTrigger) {
        const logFooterHeight = 20
        let index = 0
        this.logOffsetTop.forEach((item, logIndex) => {
          if (item.content && (scrollTop + this.logContentHeight > item.top + logFooterHeight)) {
            index = logIndex
          }
        })
        if (index < 0) {
