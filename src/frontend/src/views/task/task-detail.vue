  ...
  <div class="task-detail-wrapper" v-bkloading="{ isLoading: loading }">
      ...
      <section class="task-table" v-bkloading="{ isLoading: tableLoading }">
        ...
        <bk-table
          ...
          <bk-table-column
            ...
            :resizable="false">
            <template #default="{ row }">
              <!-- 命令查看 手动类型 && ( 安装 >=2 | 重装 >= 1 | 升级 >=1 (仅windows有) | 卸载 无命令 ) -->
    ...
    <bk-sideslider
      ...
      <div slot="content" class="commands-wrapper" v-bkloading="{ isLoading: commandLoading }">
        ...
        <p class="guide-title" v-else>{{ $t('slider安装title', { type: operateHost }) }}</p>
        <p class="guide-title">
...
import StrategyTemplate from '@/components/strategy/strategy-template'
...
export default {
      ...
      filterData: [
        { name: 'IP', type: 'innerIp', id: 'ip' },
        {
          ...
          children: [
            ...
            { id: 'FILTERED', name: this.$t('已忽略'), checked: false }
          ]
        }
      ],
      pagination: {
  ...
  computed: {
    ...
    // 是否为安装类型
    isInstallType() {
      return this.installReg.test(this.jobType)
    },
  ...
  beforeRouteLeave(to, from, next) {
    this.setToggleDefaultContent(false) // 带返回的路由背景置为白色
    next()
  },
  methods: {
    ...mapMutations(['setToggleDefaultContent']),
    ...mapActions('task', ['requestHistoryTaskDetail', 'requestTaskStop', 'requestTaskRetry', 'requestCommands']),
    ...mapActions('agent', ['getCloudList']),
    /**
     * 拉取主机执行结果统计数量 及 主机列表
     */
    async getHistoryTaskDetail() {
      this.tableLoading = true
      const res = await this.requestHistoryTaskDetail({
        id: this.taskId,
    ...
    async handleTaskRetry(selction = []) {
      ...
      this.setRowsLoading(data, true)
      const res = await this.requestTaskRetry({
        jobId: this.taskId,
        params: { instance_id_list: data.map(item => item.instanceId) }
      })
    ...
    async requestCommandData(row) {
      if (!this.commandLoading) {
        ...
        const res = await this.requestCommands({
          jobId: this.taskId,
          params: {
          ...
          if (row) {
            ...
            Object.keys(res).forEach((key) => {
              ...
              if (parseInt(key) === window.PROJECT_CONFIG.DEFAULT_CLOUD) {
                ...
                const cloud = this.cloudList.find(item => `${item.bkCloudId}` === key
    ...
    async copyCommand(row, index) {
      ...
      if (row) {
        ...
        commandStr = this.commandData.map(item => item.totalCommands.replace(/<[^>]+>/gi, '')).join('\n')
      }
    ...
    viewCommand(row) {
      ...
      this.slider.title = this.$t('手动安装sliderTitle', { ip: row ? row.innerIp : '' })
