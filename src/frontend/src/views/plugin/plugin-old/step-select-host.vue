  ...
  <div :class="['plugin-steps', { 'footer-fixed': isScroll && table.data.length }]">
    ...
    <section class="agent-content" v-bkloading="{ isLoading: loading }">
      <bk-table
        ...
        <template #prepend>
          <transition name="tips">
            <div class="selection-tips" v-show="isAllChecked && selectionCount">
              ...
              <div v-if="!isSelectedAllPages">
                {{ $t('共') }}
                <span class="tips-num">{{ table.pagination.count }}</span>
                {{ $t('条') }}
              </div>
              <bk-button ext-cls="tips-btn" text v-else @click="handleClearSelection">{{ $t('清除所有数据') }}</bk-button>
            </div>
          </transition>
        </template>
        ...
        <bk-table-column class-name="row-ip" min-width="50" :label="$t('节点类型')" prop="node_type" :resizable="false">
          <template #default="{ row }">
...
export default {
  ...
  computed: {
    ...mapGetters(['cloudList', 'permissionSwitch']),
    ...mapGetters('agent', ['cloudList']),
    ...mapGetters(['bkBizList', 'selectedBiz', 'fontSize']),
    // 当前列表选择数据
    selection() {
  ...
  methods: {
    ...mapActions(['getBkBizPermission']),
    ...mapActions('plugin', ['getHostList', 'getFilterCondition']),
    async handleInit() {
      this.loading = true
      await this.getFilterCondition().then((data) => {
    ...
    async getCheckedCount() {
      ...
      const res = await this.getHostList(params)
    ...
    async getBizOperaPermission() {
      const list = await this.getBkBizPermission({ action: 'plugin_operate' })
