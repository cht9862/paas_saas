import { IUserConfig } from '@/types/index'
export default class RequestQueue {
  private queue: IUserConfig[] = []
  public constructor() {
    this.queue = []
  }
   ...
   */
  public set(newRequest: IUserConfig) {
  ...
  /**
     ...
     */
  public cancel(requestIds: string[] | string, msg = 'request canceled') {
    let cancelQueue: IUserConfig[] = []
    ...
    try {
      cancelQueue.forEach((request) => {
        ...
        request.cancelExcutor && request.cancelExcutor(`${msg}: ${requestId}`)
      })
