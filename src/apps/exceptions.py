...
class BkJwtClientException(BaseException):
    ERROR_CODE = 16
    MESSAGE = _("请升级blueapps至最新版本")


class BkJwtVerifyException(BaseException):
    ERROR_CODE = 17
    MESSAGE = _("获取JWT信息异常")


class BkJwtVerifyFailException(BaseException):
    ERROR_CODE = 18
    MESSAGE = _("JWT校验失败")


class AuthOverdueException(BaseException):
    ERROR_CODE = 19
