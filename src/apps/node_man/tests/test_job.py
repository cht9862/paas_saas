...
class TestJob(TestCase):
    ...
    def test_job_list(self):
        ...
        # 带业务参数查询
        JobHandler().list(
            {"page": 1, "pagesize": 10, "bk_biz_id": [2], "sort": {"sort_type": "ASC", "head": "total_number"}}, "admin"
        )
        ...
        JobHandler().list(
            {"page": 1, "pagesize": 10, "job_id": [9999], "sort": {"sort_type": "DEC", "head": "total_number"}}, "admin"
        )

        # 全业务查询
        JobHandler().list(
            {"page": 1, "pagesize": 10, "bk_biz_id": [biz["bk_biz_id"] for biz in SEARCH_BUSINESS]}, "admin"
        )
    ...
    def test_gen_commands(self):
        ...
        commands = JobHandler(job_id=job_id).get_commands("admin", -1)
