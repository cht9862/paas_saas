...
class TestHost(TestCase):
    ...
    def test_host_update(self):
        ...
        host_to_create, identity_to_create, _ = create_host(1, ip="255.255.255.253", bk_host_id=654645645)
        # Cloud信息的CMDB校验
        update_data = {"bk_host_id": host_to_create[0].bk_host_id, "bk_cloud_id": 5, "outer_ip": "23.1.2.3"}
        HostHandler().update(update_data, "admin", True)
        self.assertEqual(
        ...
        # 外网占用
        create_host(number=1, outer_ip="111.111.111.111", bk_cloud_id=5, bk_host_id=9999)
        update_data = {"bk_host_id": host_to_create[0].bk_host_id, "outer_ip": "111.111.111.111", "bk_cloud_id": 5}
        self.assertRaises(IpInUsedError, HostHandler().update, update_data, "admin", True)

        # 登录IP占用
        create_host(number=1, login_ip="111.111.111.111", bk_cloud_id=5, bk_host_id=10000)
        update_data = {"bk_host_id": host_to_create[0].bk_host_id, "login_ip": "111.111.111.111", "bk_cloud_id": 5}
        self.assertRaises(IpInUsedError, HostHandler().update, update_data, "admin", True)
