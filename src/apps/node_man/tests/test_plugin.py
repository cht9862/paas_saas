...
from apps.node_man.models import ProcessStatus, Packages
...
class TestPlugin(TestCase):
    ...
    def test_plugin_list_search(self):
        ...
        PluginHandler.list({"pagesize": page_size, "page": 1, "only_ip": False})
    ...
    def test_plugin_list_span_page(self):
        ...
        hosts = PluginHandler.list(
            {"exclude_hosts": bk_host_ids[:100], "pagesize": -1, "page": 1, "only_ip": False, "running_count": True},
        )
        for host in hosts["list"]:
            self.assertIn(host["bk_cloud_id"], [0, 1])
        ...
        PluginHandler.operate(
            {
                ...
                "bk_host_id": [1],
                "plugin_params": {"name": "pluginplugin", "version": "latest"},
            },
            "admin",
            True,
        )
