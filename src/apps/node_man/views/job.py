...
class JobViewSet(ModelViewSet):
    ...
    def install(self, request):
        ...
        return Response(
            JobHandler().job(self.validated_data, get_request_username(), request.user.is_superuser, ticket,)
        )
    ...
    def retry(self, request, *args, **kwargs):
        ...
        """
        return Response(
            JobHandler(job_id=kwargs["pk"]).retry(request.data.get("instance_id_list", []), get_request_username())
        )
    ...
    def get_job_commands(self, request, *args, **kwargs):
        ...
        return Response(
            JobHandler(job_id=kwargs["pk"]).get_commands(
                username=get_request_username(), request_bk_host_id=validated_data["bk_host_id"]
            )
        )
