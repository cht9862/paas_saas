...
from apps.node_man.models import Host, SubscriptionInstanceRecord, SubscriptionTask, Subscription, ProcessStatus
...
class DebugHandler(APIModel):
    ...
    def task_details(self, subscription_id, task_id):
        """
        获得任务执行详情
        ...
        instance_records = SubscriptionInstanceRecord.objects.filter(subscription_id=subscription_id, task_id=task_id)
        # 获取任务下所有主机的详细执行信息
        instances_info = []
        for instance_info in instance_records:
            instance_temp_info = {}
            instance_temp_info["subscription_id"] = subscription_id
            instance_temp_info["task_id"] = task_id
            instance_temp_info["instance_id"] = instance_info.instance_id
            instance_temp_info["logs"] = JobHandler.get_log_base(subscription_id, instance_info.instance_id)
            instance_temp_info["create_time"] = instance_info.create_time
            instance_temp_info["update_time"] = instance_info.update_time
            instance_temp_info["is_latest"] = instance_info.is_latest
            instances_info.append(instance_temp_info)
