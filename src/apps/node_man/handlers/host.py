...
from apps.node_man.models import Host, Cloud, IdentityData, ProcessStatus, JobTask, AccessPoint
...
class HostHandler(APIModel):
    ...
    def fuzzy_cond(self, custom: str, wheres: list, sql_params: list):
        """
        用于生成模糊搜索的条件
        :param custom: 用户的输入
    ...
    def multiple_cond_sql(self, params: dict, user_biz: dict, proxy=False, plugin=False):
        """
        用于生成多条件sql查询
        :param params: 条件数据
        ...
        # 插件查询条件
        bk_host_id_list = []
        if plugin and params.get("conditions"):
            wheres = [
                ...
                f'{ProcessStatus._meta.db_table}.source_type="default"',
            ]
            for condition in params["conditions"]:

                if condition["key"] in const.HEAD_PLUGINS:
                    # 插件版本的精确搜索
                    placeholder = []
                ...
                elif condition["key"] in [f"{plugin}_status" for plugin in const.HEAD_PLUGINS]:
                    # 插件状态的精确搜索
                    placeholder = []
                    for cond in condition["value"]:
                        placeholder.append('"{}"'.format(cond))
            ...
            bk_host_id_list = set(
                Host.objects.extra(select=select, tables=[ProcessStatus._meta.db_table], where=wheres)
                .filter()
        ...
        if params.get("bk_biz_id"):
            ...
            biz_permission = [bk_biz_id for bk_biz_id in params["bk_biz_id"] if bk_biz_id in user_biz]
        else:
            # 否则返回用户所有有权限的业务的主机
            biz_permission = list(user_biz.keys())
        ...
        if params.get("conditions"):
            for condition in params["conditions"]:
                if condition["key"] in ["os_type", "inner_ip", "bk_cloud_id", "node_from"]:
                    ...
                    # 是否手动安装
                    kwargs["is_manual__in"] = condition["value"]

                elif condition["key"] == "topology":
                    # 集群与模块的精准搜索
                    biz = condition["value"].get("bk_biz_id")
                    if biz in biz_permission:
                        ...
                        if kwargs["bk_host_id__in"] is not None:
                            kwargs["bk_host_id__in"].extend(host_ids)
                        else:
                            kwargs["bk_host_id__in"] = host_ids
                    else:
                        kwargs["bk_host_id__in"] = []

                elif condition["key"] == "query" and isinstance(condition["value"], str):
                    # IP、操作系统、Agent状态、Agent版本、云区域 单模糊搜索
                    custom = condition["value"]
                    wheres, sql_params = self.fuzzy_cond(custom, wheres, sql_params)
                    ...
                    for custom in condition["value"]:
                        where_or, sql_params = self.fuzzy_cond(custom, where_or, sql_params)
        ...
        sql = (
            Host.objects.filter(node_type__in=node_type)
            .extra(select=select, tables=[ProcessStatus._meta.db_table], where=wheres, params=sql_params)
            .filter(**filter_values(kwargs))
        )
        if plugin:
            sql = sql.filter(bk_host_id__in=bk_host_id_list)

        return sql

    def multiple_cond_sql_running_number(self, queryset):
        """
    ...
    def proxies(self, params: dict, username: str, is_superuser: bool):
        ...
        for proxy in proxies:
            proxy["bk_biz_name"] = user_biz.get(proxy["bk_biz_id"])
    ...
    def biz_proxies(self, params: dict, username: str, is_superuser: bool):
        ...
        # 用户有权限的业务
        user_biz = CmdbHandler().biz_id_name(username)
