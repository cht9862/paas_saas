...
from apps.node_man.models import Cloud, AccessPoint
...
class IamHandler(APIModel):
    ...
    def fetch_biz(self):
        ...
        result = [
            ...
            for business in client_v2.cc.search_business({"fields": ["bk_biz_id", "bk_biz_name"]})["info"]
        ]
    ...
    def handle_policy_content(self, content, action_id, instance_type, username, ret):
        ...
        # 拥有单个权限
        elif content["field"] in ["biz.id", "cloud.id", "ap.id"] and content["op"] == "eq":
            ret[action_id].append(int(content["value"]))

        # 拥有部分权限
        elif content["field"] in ["biz.id", "cloud.id", "ap.id"] and content["op"] == "in":
    ...
    def fetch_redirect_url(self, params, username):
        ...
        for param in params:
            ...
            if action_type != "create":
                ...
                apply_info["related_resource_types"] = [
                    {
                        # 关联的资源类型, 无关联资源类型的操作, 可以为空
                        "system_id": system_id,
                        "type": "biz" if instance_type in ["agent", "proxy", "plugin", "task"] else instance_type,
                    }
                ]
                if instance_id:
                    apply_info["related_resource_types"][0]["instances"] = [
                        [
                            {
                                # 层级节点的资源类型
                                "type": "biz"
                                if instance_type in ["agent", "proxy", "plugin", "task"]
                                else instance_type,
