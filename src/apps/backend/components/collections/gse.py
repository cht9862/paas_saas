...
class GseBaseService(six.with_metaclass(abc.ABCMeta, Service)):
    ...
    def execute(self, data, parent_data):
        ...
        if result["failed"]:
            ...
            self.log_error(
                "GSE register process failed. result:\n[{}]".format(json.dumps(result, indent=2)), log_context,
            )
        ...
        self.log_info(
            "GSE {} Process and get task_id: [{}]".format(self.ACTION.upper(), task_id), log_context,
        )
    ...
    def schedule(self, data, parent_data, callback_data=None):
        ...
        if is_finished:
            ...
            if task_result["failed"]:
                ...
                return False
            self.log_info(
                "GSE(task_id: [{}]) get schedule finished].".format(task_id), log_context,
            )
            return True
    ...
    def schedule(self, data, parent_data, callback_data=None):
        ...
        if not is_finished:
            if polling_time > POLLING_TIMEOUT:
                self.log_error(
                    "GSE(task_id: [{}]) schedule polling timeout.".format(task_id), log_context,
                )
                data.outputs.ex_data = "任务轮询超时"
                return False
        ...
        if data.outputs.action == "check":
            ...
            if not_exist_hosts:
                ...
                self.log_info(
                    "GSE check process exists. result:\n[{}]".format(json.dumps(task_result, indent=2)), log_context,
                )
                # 进程存在，则重载
    ...
    def schedule(self, data, parent_data, callback_data=None):
        ...
        if is_finished:
            ...
            for fail in task_result["failed"]:
                ...
                self.log_error(
                    "GseStopProcessService stop process failed:\n[{}]".format(json.dumps(fail, indent=2)), log_context,
                )
            if failed_task_result:
                ...
                return False
            # 注销进程结果处理
            result = gse_client.unregister_process(hosts, proc_name)
            failed_results = []
            for fail in result["failed"]:
                if fail["error_code"] == GseDataErrCode.NON_EXIST:
                    continue
                failed_results.append(fail)
            if failed_results:
                data.outputs.result = result  # result字段存注销结果
                data.outputs.ex_data = "以下主机注销进程失败：{}".format(
                    ",".join(["[{}] {}".format(host["ip"], host.get("error_msg")) for host in failed_results])
                )
                self.log_error(
                    "GSE unregister process failed. result:\n[{}]".format(json.dumps(result, indent=2)), log_context,
                )
                return False
            self.log_info("GSE unregister process success.", log_context)

        elif polling_time + POLLING_INTERVAL > POLLING_TIMEOUT:
            self.log_error(
                "GseStopProcessService(task_id: [{}]) schedule timeout.".format(task_id), log_context,
            )
