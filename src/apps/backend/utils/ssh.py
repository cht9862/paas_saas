...
# paramiko.util.log_to_file(os.path.join(settings.BK_LOG_DIR, 'paramiko_log.txt'))
...
def ssh_login(
    ip, port, auth_type, account="root", password=None, key=None, timeout=SSH_CON_TIMEOUT, _logger=None,
) -> paramiko.SSHClient:
    """
    使用ssh登录
    :param _logger:
    ...
    try:
        ...
        SshMan.safe_close(ssh)
        msg = "SSH authentication failed.- {}@{}:{} - exception: {}".format(account, ip, port, e,)
        msg = "{}, {}".format(msg, _("登录认证失败，请确认账号，密码或密钥是否正确。"))
        _logger.error(msg)
        raise e  # 密码错或者用户错(Authentication failed)
    except paramiko.SSHException as e:
        SshMan.safe_close(ssh)
        msg = "SSH connect failed.- {}@{}:{} - exception: {}".format(account, ip, port, e)
        msg = "{}, {}".format(msg, _("ssh登录，请确认目标机器是否可被正常登录。"))
        _logger.error(msg)
        ...
        msg = "{}, {}".format(msg, _("ssh登录连接超时，请确认ssh端口号是否正确或网络策略是否正确开通。"))
        _logger.error(msg)
        raise e  # 超时
    else:
        return ssh
...
class SshMan(object):
    ...
    def __init__(self, host: Host, log):
        ...
        if (identity_data.auth_type == AuthType.PASSWORD and not identity_data.password) or (
            ...
            log.info(_(f"认证信息已过期, 请重装并填入认证信息"))
    ...
    def send_cmd(
        self, cmd, wait_console_ready=True, is_adding_output=False, is_clear_cmd_and_prompt=True, check_output=True,
    ):
        """
        用指定账户user发送命令cmd
