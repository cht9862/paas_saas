...
def gen_commands(host, pipeline_id, is_uninstall, batch_install=False):
    ...
    if host.node_type == constants.NodeType.AGENT:
        gse_servers = ",".join(server["inner_ip"] for server in host.ap.taskserver)
        package_url = host.ap.package_inner_url
        callback_url = settings.BKAPP_NODEMAN_CALLBACK_URL
    else:
        gse_servers = ",".join(server["outer_ip"] for server in host.ap.taskserver)
        package_url = host.ap.package_outer_url
        callback_url = settings.BKAPP_NODEMAN_OUTER_CALLBACK_URL
    ...
    if host.node_type in [constants.NodeType.AGENT, constants.NodeType.PROXY] and not batch_install:
        ...
        if host.os_type == constants.OsType.WINDOWS:
            ...
            run_cmd_params.append(f'-n "{gse_servers}"')
            if is_uninstall:
                # 卸载操作
        ...
        if not batch_install:
            ...
            run_cmd_params.extend(
                [f"-n {','.join(proxies)}", f"-I {proxy.inner_ip}", f"-o http://{proxy.inner_ip}:17980/"]
            )
        else:
            run_cmd_params.extend([f"-n {gse_servers}", f"-i {host.bk_cloud_id}", f"-I {host.inner_ip}", "-N SERVER"])
        if is_uninstall:
            # 卸载操作
            run_cmd_params.append("-R")
        run_cmd = (
