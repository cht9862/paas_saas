...
from apps.exceptions import ApiResultError, ApiRequestError, PermissionError, BaseException
from apps.utils.local import get_request_id, get_request, get_request_username
...
class DataAPI(object):
    ...
    def __call__(self, params=None, files=None, raw=False, timeout=None, raise_exception=True):
        ...
        try:
            ...
            if raise_exception and not response.is_success():
                raise ApiResultError(response.message, code=response.code, errors=response.errors)
    ...
    def _send_request(self, params, timeout, request_id):
        ...
        try:
            try:
                raw_response = self._send(params, timeout, request_id)
            ...
            if raw_response.status_code != self.HTTP_STATUS_OK:
                error_message = "data api response status code error code->[%s] url->[%s] content->[%s]" % (
    ...
    def _send(self, params, timeout, request_id):
        ...
        if self.method.upper() == "GET":
            ...
            if not file_data:
                try:
                    csrftoken = get_request().META.get("HTTP_X_CSRFTOKEN")
                except BaseException:
                    csrftoken = ""
                session.headers.update({"Content-Type": "application/json; chartset=utf-8", "X-CSRFtoken": csrftoken})
...
class PassThroughAPI(DataAPI):
    ...
    def __init__(self, module, method, url_prefix, sub_url, supported_api=None):
