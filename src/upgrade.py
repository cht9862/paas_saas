...
DB_HOST = os.environ.get("MYSQL_IP0", "localhost")
...
try:
    ...
    print("[Error---->Usage example]: python upgrade.py $APP_CODE $APP_SECRET [$PAAS_FQDN]")
    exit(1)

PAAS_HOST = os.environ.get("PAAS_HOST", "")
SEARCH_BIZ_URL = "http://{}/api/c/compapi/v2/cc/search_business/".format(PAAS_HOST or sys.argv[3])
LIST_BIZ_HOSTS_URL = "http://{}/api/c/compapi/v2/cc/list_biz_hosts/".format(PAAS_HOST or sys.argv[3])
...
def migrate_cloud_creator(clouds):
    for cloud in clouds:
        if not cloud["bk_biz_id"] or cloud["bk_biz_id"] == -1:
            continue
...
if __name__ == "__main__":
    ...
    try:
        ...
        cursor.execute("INSERT INTO django_migrations(app, name, applied) VALUES ('node_man', '0001_initial', now());",)
